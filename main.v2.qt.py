...
def run_proto_clip(cfg, visual_memory_keys, visual_memory_values, val_loader, test_loader, textual_memory_bank, clip_model, text_prompts, train_loader_F):
    ...
    if not cfg['only_test']:
        ...
        for epoch in tqdm(range(cfg['train_epoch'])):
            ...
            for i, (images, target) in tqdm(enumerate(train_loader_F)):
                query_images, zq_labels = images.cuda(), target.cuda()

                p, z_img_proto, z_text_proto = model(query_images)
            ...
            with torch.no_grad():
                ...
                for i, (images, target) in tqdm(enumerate(val_loader)):
                    ...
                    val_images, val_labels = images.cuda(), target.cuda()
                    ...
                    val_correct += (p.max(1)[1] == val_labels).float().sum()
    ...
    for i, (images, target) in tqdm(enumerate(test_loader)):
        ...
        test_images, test_labels = images.cuda(), target.cuda()
        ...
        test_correct = (p.max(1)[1] == test_labels).float().sum()

    test_acc = test_correct / test_correct
...
def main():
    ...
    # ------------------------------------------ Proto-CLIP ------------------------------------------
    run_proto_clip(cfg, visual_memory_keys, visual_memory_values, val_loader, test_loader, textual_memory_bank, clip_model, text_prompts, train_loader_F)
