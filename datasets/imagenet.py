...
def get_random_train_tfm():
    """
    Get a random set of training transforms.

    Returns:
        torchvision.transforms.Compose: Composed set of transforms.
    """
    return transforms.Compose([
        ...
        transforms.Normalize(mean=(0.48145466, 0.4578275, 0.40821073), std=(
            0.26862954, 0.26130258, 0.27577711))  # Normalize image
    ])
...
class ImageNet():
    ...
    def __init__(self, root, num_shots, preprocess):
        ...
        train_preprocess = get_random_train_tfm()
        test_preprocess = preprocess

        self.train = torchvision.datasets.ImageNet(
            self.image_dir, split='train', transform=train_preprocess)
        self.val = torchvision.datasets.ImageNet(
            self.image_dir, split='val', transform=test_preprocess)
        ...
        for i in range(len(self.train.imgs)):
            split_by_label_dict[self.train.targets[i]].append(
                self.train.imgs[i])
        imgs = []
        targets = []
