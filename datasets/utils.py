...
class DatasetWrapper(TorchDataset):
    ...
    def __getitem__(self, idx):
        ...
        self.text_data.append(f"{item.impath}")
        if len(self.text_data)==3168 and self.is_train:
            with open("./id_mapping_198.txt", "w+") as f:
                f.write("\n".join(self.text_data))
...
class QueryDataset(TorchDataset):
    def __init__(self, n, k):
        ...
        for i in range(self.k):
            bin_min = i * self.n // self.k
            bin_max = (i + 1) * self.n // self.k
            bin_data = [random.uniform(bin_min, bin_max)
                        for j in range(bin_max - bin_min)]
...
def collate_fn(batch):
    # Convert a list of samples into a batch
    return torch.tensor(batch, dtype=torch.float32)


def create_dataloader(n, k, batch_size):
    dataset = QueryDataset(n, k)
