...
def get_arguments():
    ...
    parser.add_argument('--use_gpt_prompts', dest='use_gpt_prompts',
                        help='use gpt generated text prompts', action='store_true')
...
def populate_cfg_using_args(cfg, args):
    ...
    cfg['only_test'] = args.only_test
    cfg['use_gpt_prompts'] = args.use_gpt_prompts
...
def main():
    ...
    if cfg['use_gpt_prompts']:
        with open(f"./gpt_file/{cfg['dataset']}_prompt.json") as f:
    ...
    text_prompts, textual_memory_bank = get_textual_memory_bank(
        cfg, dataset.classnames, dataset.template, clip_model, gpt3_prompt)
